(ns fja_sapiens.main-test
  "Contract + behavioral test for the fja_sapiens-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "fja_sapiens-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'fja_sapiens.main-test)]
