(ns fja_sapiens.main
  "Kotodama WASM entrypoint for the Fja_sapiens clean-room actor (L4) — Clojure port.
  ...
  facts (`fja_sapiens.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "fja_sapiens")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "fjasapie_acc"
    :fields [:holderId :type :balance :currency :status] :required [:type :balance]
    :coerce {:balance :float}          :refs {}}
   {:entity "Loan"        :plural "loans"        :id-prefix "fjasapie_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"fja_sapiens.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "fja_sapiens-compat" :tier tier :entities entities} 200])
...
(defn start! [] :fja_sapiens-compat/ready)
